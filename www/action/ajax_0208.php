...
100001원 부터는 단폴배팅 안된다는 메시지 해주셨으면 합니다.*/
		...
		case 'ladderChk':
			if($_POST[glkey]==1326){
				...
				$que = "SELECT COUNT(*) FROM gamelist a, buygamelist b WHERE b.M_Key = '{$_SESSION[S_Key]}'  AND a.GL_Key = {$gl_key} AND a.G_Datetime = '{$_POST[gtime]}' AND a.G_Key = b.G_Key";
				//echo $que;
				$row = getRow($que);
			}
			echo $row[0];
