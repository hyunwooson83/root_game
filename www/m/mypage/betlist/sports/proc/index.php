...
switch($mode){
    ...
    case 'bettingCancel':
        ...
        //print_r($meminfo);
        //echo $meminfo['M_Bet_Cancel_Cnt'];
        #회원배팅취소가능 횟수가 0이상일 경우만
        if($meminfo['M_Bet_Cancel_Cnt']>0){
            $que = "SELECT BG_BettingPrice FROM buygame WHERE BG_Key = '{$bgkey}'";
            ...
            if($end_time < 0 && $SITECONFIG['member_bet_cancel_YN']=='Y'){
                $fail++;
                $json['flag'] = false;
                $json['error'] = "배팅취소시간이 경과하여 배팅을 취소하실 수 없습니다.";
                echo json_encode($json);
                break;
            }
