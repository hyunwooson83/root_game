...
?>
    <div id="sub_wrap">
        ...
        <div class="sports_list">
            ...
            if(count($arr)>0){
            foreach($arr as $rs) {
            $sql = "SELECT *, a.G_Key as bgkey FROM buygamelist a LEFT JOIN gamelist b ON a.G_Key = b.G_Key LEFT JOIN gameleague c ON b.GL_Key = c.GL_Key_IDX WHERE a.M_Key = '{$_SESSION['S_Key']}' AND a.BG_Key = '{$rs['BG_Key']}' ORDER BY G_Datetime ASC";
            ...
            if(count($ar)>0){
                ...
                if(in_array($list['BGL_ResultChoice'],array('Win','Draw','Lose'))){
                    ...
                    $rate_draw = number_format($list['BGL_QuotaDraw'],2);
            ...
            <!-- League start { -->
            <h1>
                <span><img src="/img/icon_<?php echo $ITEMICON[$list['GI_Key']];?>.png" /></span>
                <?php if(!in_array($list['BGL_QuotaWin'],array(1.03,1.05,1.07))){ ?>
                <img src="/img/league/<?php echo $list['GL_SrvName']; ?>" />
                <?php } ?>
                <?php echo $list['GL_Type']; ?>
                <var><?php echo date("m/d H:i",strtotime($list['G_Datetime'])); ?> <?php echo ($list['G_Type1']=='Special')?$mkt:''; ?></var>
                <em>
                    <span><?php
            ...
            <fieldset>
                ...
                <div>
                    ...
                    <?php } ?>
                     &nbsp;<a href="javascript:;" class="betting-upload">내역올리기</a>
                </div>
            </fieldset>
                <?php $cnt++; }} else { ?>
                <ul>
