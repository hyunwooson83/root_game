...
$where = " 1 AND c.GL_Key IN (38919) AND DATE_FORMAT(BG_BuyDate,'%Y-%m-%d') BETWEEN '{$startDate}' AND '{$endDate}' AND BG_Visible = 1 AND b.M_Key = '{$_SESSION['S_Key']}' ";
    ...
    <div id="sub_wrap">
        <div class="sub_title">
            <ul class="sub_title_category">
                ...
                <li onClick="location.href='/m/mypage/betlist/virtual/soccer/'" class="active">가상게임</li>
            </ul>
            <h1>
                <span>가상개경주 베팅내역</span>
                <em>VIRTUAL HORSE BETTING LIST</em>
            </h1>
        </div>
        ...
        <div class="sub_mypage_wrap">
            ...
            <ul id="type_select_l" class="type_select_list">
                <li onclick="location.href='/m/mypage/betlist/virtual/soccer/'">가상축구</li>
                <li onclick="location.href='/m/mypage/betlist/virtual/horse/'">가상경마</li>
                <li onclick="location.href='/m/mypage/betlist/virtual/dog/'" class="active">가상개경주</li>
            </ul>
            <!-- 경기검색 { -->
        ...
        <table class="sub_board_list sub_board_list_betlist">
            <thead>
            <tr>
                <td>파워사다리 베팅내역 <dfn>1,274</dfn></td>
            </tr>
            </thead>
            <tbody>
            <?php
            if($total_article > 0){
                ...
                foreach($rs as $rs){
                    ...
                    if($rs['BGL_ResultChoice'] == 'Win'){
                        $betting_text = "<span style='color:dodgerblue'>승</span>";
                    } else if($rs['BGL_ResultChoice']=='Draw'){
                        $betting_text = "<span style='color:dodgerblue'>무</span>";
                    } else if($rs['BGL_ResultChoice']=='Lose'){
                        $betting_text = "<span style='color:dodgerblue'>패</span>";
                    } else if($rs['BGL_ResultChoice']=='Over'){
                        $betting_text = "<span style='color:dodgerblue'>오버</span>";
                    } else if($rs['BGL_ResultChoice']=='Under'){
                    ...
                    <tr>
                        <td>
                            <!-- 베팅내역 { -->
                            <div>
                                <dfn class="font_gobet">No. <?php echo $rs['BG_Key'];?> &nbsp; <span>가상개경주</span></dfn>
                                <label><span class="ing"></span></label>
                            </div>
                            <fieldset>
                                <em><input type="checkbox" name="bgkey[]" class="bgkey" value="<?php echo $rs['BG_Key']; ?>"></em>
                                <dl>
                                    <dt>베팅내역</dt>
                                    <dd class="blue"><?php echo $rs['BGL_ResultChoice'];?>번</dd>
                                    <dt>적중상태</dt>
                                    <dd class="<?php echo $result_css;?>"><?php echo $rseult_text;?></dd>
                                </dl>
                                <dl>
                                    <dt>게임회차</dt>
                                    <dd><?php echo $rs['G_Num'];?>회</dd>
                                    <dt>게임구분</dt>
                                    <dd>가상개경주</dd>
                                </dl>
                                <dl>
