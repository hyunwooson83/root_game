...
?>
        ...
        <div class="sub_wrap type2">
            ...
            <div class="sub-box">
                ...
                <div class="mypage_menu2">
                    <a href="/mypage/betlist/sports/" class="active">스포츠게임</a>
                    <a href="/mypage/betlist/live/" >라이브</a>
                ...
                <div class="board_wrap">
                    ...
                    <table class="table-black mypage">
                        ...
                        if(count($arr)>0){
                            foreach($arr as $rs) {
                                $sql = "SELECT *, a.G_Key as bgkey FROM buygamelist a LEFT JOIN gamelist b ON a.G_Key = b.G_Key LEFT JOIN gameleague c ON b.GL_Key = c.GL_Key_IDX WHERE a.M_Key = '{$_SESSION['S_Key']}' AND a.BG_Key = '{$rs['BG_Key']}' ORDER BY a.G_Key ASC";
                                //echo $sql;
                                ...
                                if(count($ar)>0){
                                    foreach($ar as $list){
                                        ...
                                        } else if($list['G_State'] == 'Stop'){
                                            $g_state = "경기중";
                                            $g_state_css = "";
                                            $game_state++;
                                        } else if($list['G_State'] == 'Await') {
                                            $g_state = "경기전";
                                            $g_state_css = "";
                                            $game_state++;
                                        ...
                                        <tr class="line">
                                            <td>
                                                <?php if(in_array($list['BGL_QuotaWin'],array(1.03,1.05,1.07))){
                                                    echo "보너스";
                                                } else {
                                                    ?>
                                                    <?php echo $GAME_TYPE_TEXT[$list['G_Type2']];?> <?php echo ($list['G_Type1']=='Special')?$mkt:''; ?>
                                                <?php } ?>
                                            </td>
                                    ...
                                    <td colspan="7" class="table-left">
                                        ...
                                        <span>
                                            ...
                                            <em>
                                                ...
                                                if($end_time>0 && $SITECONFIG['member_bet_cancel_YN']=='Y' && $meminfo['M_Bet_Cancel_Cnt']>0){
                                                    ?>
                                                    <font class="betting-cancel" data-bgkey="<?php echo $rs['BG_Key'];?>">배팅취소</font>
                                                <?php } ?>
                                            </em>
                                        </div>
                                    </td>
    ...
    <form name="betcancel" method="post" action="./proc/" style="display: none;" target="hiddenFrame">
        <input type="text" name="HAF_Value_0" id="mode" value="">
        <input type="text" name="bgkey" id="bgkey" value="">
    </form>
    <iframe src="" style="display: none" name="hiddenFrame" frameborder="0"></iframe>
    <script>
        $(document).ready(function(){
            ...
            $('.betting-cancel').on('click',function(){
                ...
                swal({
                    ...
                    if (isConfirm) {
                        var f = document.betcancel;

                        $('#mode').val('bettingCancel');
                        $('#bgkey').val(bgkey);
                        f.submit();
                    }
                });
        ...
        function roll_back(res){
            if(res == true){
                swal('','배팅이 정상적으로 취소되었습니다.','success');
                setTimeout(function(){ location.reload(); }, 2000);
            } else {
                swal('','배팅취소시 오류가 발생했습니다. 관리자에게 문의해주세요.','warning');
                setTimeout(function(){ location.reload(); }, 3000);
            }
        }
