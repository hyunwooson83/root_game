...
switch($_POST['HAF_Value_0'])
{
    ...
    case 'BuyCart':
        ...
        $bettime = strtotime("+50 seconds",strtotime(date("Y-m-d H:i:s")));

        //echo $bettime."->".$gametime;
        if($bettime > $gametime) $lib->AlertMsg("배팅가능한 시간이 아닙니다. 다음 게임을 이용해주세요.");
        ...
        if($game_gubun=='PB'){
            ...
            $que .= "BG_GameGubun		= 3, ";
        } else  if($game_gubun=='soccerw'){
            $que .= "BG_GameGubun		= 4, ";
        } else if($game_gubun=='horse'){
            $que .= "BG_GameGubun		= 5, ";
        } else if($game_gubun=='dog'){
            $que .= "BG_GameGubun		= 6, ";
        ...
        /*if(in_array($game_gubun,array('PBL','PB'))){
            $sql .= "BGL_Odd		 		= {$rate[0]}, ";
            $sql .= "BGL_Even		 		= {$rate[1]}, ";
        }*/
        ...
        } else if($game_gubun == 'soccerw'){//가상스포츠 월드컵
            switch ($bet_gubun) {
                case 'Win':
                    $choice = 'Win';
                    $sql .= "BGL_QuotaWin    		= {$rate[0]}, ";
                    $sql .= "BGL_QuotaDraw	 		= {$rate[1]}, ";
                    $sql .= "BGL_QuotaLose	 		= {$rate[2]}, ";
                    break;
                case 'Draw':
                    $choice = 'Draw';
                    $sql .= "BGL_QuotaWin    		= {$rate[0]}, ";
        ...
        } else if($game_gubun == 'horse' || $game_gubun == 'dog'){//가상스포츠 월드컵
            $choice = $bet_gubun;
        }
    ...
    case 'virtualTimer':
        ...
        if(!empty($row['G_Datetime'])){
            $json['soccer_time'] = strtotime($row['G_Datetime'])-time();
        }

        $que = "SELECT G_Datetime FROM gamelist_other WHERE G_Datetime > NOW() AND GL_Key = 38919 ORDER BY G_Datetime ASC LIMIT 1";
        $row = getRow($que);
        if(!empty($row['G_Datetime'])){
            $json['horse_time'] = strtotime($row['G_Datetime'])-time();
        }

        $que = "SELECT G_Datetime FROM gamelist_other WHERE G_Datetime > NOW() AND GL_Key = 38920 ORDER BY G_Datetime ASC LIMIT 1";
        $row = getRow($que);
        if(!empty($row['G_Datetime'])){
            $json['dog_time'] = strtotime($row['G_Datetime'])-time();
        }
