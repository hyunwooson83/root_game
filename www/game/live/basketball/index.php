                ...
                <div class="score_board_contents">
                    ...
                    <div class="bl_live_betting_middle cross" style="width: 960px; display: flex; justify-content: start;">
                        ...
                        <div class="left-game-list">
                            ...
                            $que = "SELECT * FROM gamelist_live  WHERE G_State = 'Await' AND status = '1' AND matchDateTime < NOW() AND GI_Key = '48242'";
                            //echo $que;
                            $arr = getArr($que);
                            $arr_cnt = count($arr);
                            if($arr_cnt>0){
                                foreach($arr as $rs){
                                    ...
                                    if(count($list)>0){
                                        foreach($list as $list){
                                            if($list['type']=='00') $market_code_wdl = $list['marketCode']; $maeket_cnt++;
                                            if($list['type']=='02') $market_code_handi = $list['marketCode']; $maeket_cnt++;
                                            if($list['type']=='05') $market_code_ou = $list['marketCode'];  $maeket_cnt++;
    ...
    <script>
        ...
        function get_live_score(){
            $.ajax({
                ...
                async: true,
                dataType : 'json',
                data : 'mode=gameLiveScore&item=basketball',
                beforeSend : function(){
        ...
        function get_data(gid, wdl, handicap, ou, gkey){
            ...
            $.ajax({
                ...
                async: true,
                dataType : 'json',
                data : 'gid='+gid+'&mode=gameSelected&gkey='+gkey+'&item=basketball',
                success : function(data){
                    //console.log(data)
                        ...
                        if (data[0]['timeStatus'] == 3) {
                            ...
                            if(typeof data[0]['markets']['<?php echo $market_code_wdl;?>'] != 'undefined') {
                                ...
                                if (data[0]['markets']['<?php echo $market_code_wdl;?>']['suspended'] == false || inArray(data[0]['markets']['<?php echo $market_code_handi;?>']['matchOdds'][0]['option'],stop_flag) == false) {
                                    if(data[0]['markets']['<?php echo $market_code_wdl;?>']['matchOdds'][0]['option'] > 1.12) {
                                        var home_rate = rate_point2(parseFloat(rate_point2(data[0]['markets']['<?php echo $market_code_wdl;?>']['matchOdds'][0]['odds']))-minus_rate);
                                        var away_rate = rate_point2(parseFloat(rate_point2(data[0]['markets']['<?php echo $market_code_wdl;?>']['matchOdds'][1]['odds']))-minus_rate);
                                        ...
                                        if (wdl_box.length == 0) {
                                            bet_two += '<tr id="markets' + market_code_wdl + '_' + gid + '">';
                                            bet_two += '    <td colspan="5" style="width: 100%; padding-left:15px; padding-right:15px;margin-bottom:10px;" height="40">';
                                            bet_two += '        <table>';
                                            bet_two += '            <tr><td colspan="2" style="width: 100%; text-align: center;  height:30px;">최종 승패결과</td></tr>';
                                            bet_two += '            <tr>';
                                            bet_two += '                <td class="two-bet-btn ">';
                                            bet_two += '                    <div class="bl_btn bl_text_btn long betting-btn-live win-' + gid + ' live-bet home" data-code="<?php echo $market_code_wdl;?>" data-bet="Win" data-gid="' + gid + '" data-gkey="<?php echo $rs['G_Key'];?>" data-homeName="' + home_team_name + '" data-awayName="' + away_team_name + '"  >';
                                            bet_two += '                        <span class="live-left">' + home_team_name + '</span>';
                                            bet_two += '                        <span class="live-right" id="home_rate_' + gid + '">' + home_rate + '</span>';
                                            bet_two += '                    </div>';
                                            bet_two += '                </td>';
                                            bet_two += '                <td class="two-bet-btn" >';
                                            bet_two += '                    <div class="bl_btn bl_text_btn long betting-btn-live lose-' + gid + ' live-bet away" data-code="<?php echo $market_code_wdl;?>" data-bet="Lose" data-gid="' + gid + '" data-gkey="<?php echo $rs['G_Key'];?>" data-homeName="' + home_team_name + '" data-awayName="' + away_team_name + '" data-type="패" style="margin-right:3px;">';
                                            bet_two += '                        <span class="live-left">' + away_team_name + '</span>';
                                            bet_two += '                        <span class="live-right" id="away_rate_' + gid + '">' + away_rate + '</span>';
                                            bet_two += '                    </div>';
                                            bet_two += '                </td>';
                                            bet_two += '            </tr>';
                                            bet_two += '        </table>';
                                            ...
                                            bet_two += '</tr>';
                                            console.log(bet_two)
                                            $('.live-list-box.wdl').empty().append(bet_two);
                                        } else {
                                            console.log('승무패 = ' + data[0]['gid'] + ' [ ' + home_rate + ' : ' + away_rate + ']')
                                            ...
                                            $('#away_rate_' + data[0]['gid'], '.cart-rate-Lose-' + data[0]['gid']).text(away_rate).fadeOut(100).fadeIn(100).fadeOut(100).fadeIn(100);
                                        }
                                        $('.live-list-box.wdl').show();
                                        $('#markets' + market_code_wdl + '_' + gid).show();
                                    }
