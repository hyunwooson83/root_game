...
if($chkMobile == true){
    ...
    $total_item = 0;
    /*$que = "SELECT GI_Key, COUNT(DISTINCT(inPlayMatchIdx)) AS cnt FROM gamelist WHERE G_Locked = 1 AND G_Datetime > NOW() AND GI_Key IN (6046,35232,48242,154830,154914) AND G_OType1= 'G'AND G_SubType IN (60461,352321,482646,1549366,1548352) AND G_OType1 = 'G' AND G_Type1 = 'Full' AND G_Type2 = 'WDL' GROUP BY GI_Key";
    $item_cnt = getArr($que);
    ...
    $where = " 1 AND G_Datetime > NOW() AND G_QuotaWin > 0 AND G_SubType IN (60461,352321,482646,1549366,1548352) AND G_OType1 = 'G' AND G_Type1 = 'Full' AND G_Type2 = 'WDL' AND b.GL_State = 'Normal' AND G_State = 'Await'  ";
        ...
        <div class="center_con">
            <div class="score_board_category">
                <ul>
                    ...
                    <li onclick="location.href='/game/sports/WDL/'" class="active">승무패<var></var></li>
                        ...
                        if($total_article > 0){
                            ...
                            foreach($arr as $list){
                                ...
                                <tr>
                                    ...
                                    <td title="승/홈/오버">
                                        <div class="bl_btn bl_text_btn long betting-btn win-<?php echo $list['G_Key'];?>" data-bet="Win" data-gkey="<?php echo $list['G_Key']; ?>" data-glist="<?php echo $list['inPlayMatchIdx']; ?>" data-bet_cnt="<?php echo member_game_bet_cnt($_SESSION['S_Key'],$list['G_Key'],'Win');?>">
                                    ...
                                    <td class="min_space"></td>
                                    <td title="무/핸/합">
                                        <div class="bl_btn bl_text_btn middle <?php echo ($list['G_QuotaDraw']>0)?'betting-btn':''; ?> draw-<?php echo $list['G_Key'];?>" data-bet="Draw" data-gkey="<?php echo $list['G_Key']; ?>" data-glist="<?php echo $list['inPlayMatchIdx']; ?>" data-bet_cnt="<?php echo member_game_bet_cnt($_SESSION['S_Key'],$list['G_Key'],'Draw');?>"><?php echo ($list['G_QuotaDraw']>0)?$list['G_QuotaDraw']:'VS'; ?></div>
                                    </td>
                                    <td class="min_space"></td>
                                    <td title="패/원정/언더" style="padding-right:8px;" >
                                        <div class="bl_btn bl_text_btn long betting-btn lose-<?php echo $list['G_Key'];?>" data-bet="Lose" data-gkey="<?php echo $list['G_Key']; ?>" data-glist="<?php echo $list['inPlayMatchIdx']; ?>" data-bet_cnt="<?php echo member_game_bet_cnt($_SESSION['S_Key'],$list['G_Key'],'Lose');?>">
