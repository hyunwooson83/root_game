<?php
    ...
    $where = " 1 AND GI_Key = 2 AND G_State IN ('Stop','End') ";
            ...
            <div class="game_result_wrap">
                <div class="result_s_choice">
                    ...
                    <ul class="result_s_list2">
                        ...
                        <li class="active" onclick="location.href='/result/minigame/pwladder/'">파워사다리</li>
                                ...
                                <select name="turn">
                                    ...
                                    <?php
                                        $que = "SELECT G_Num2, G_Num
                                                ...
                                                AND GI_Key =2 ORDER BY G_Num DESC) A
                                                GROUP BY G_Num2";
                                        $arr = getArr($que);
                                        foreach($arr as $rs){
                                    ?>
                ...
                <div class="result_list">
                    <div class="title">
                        <span>경기결과</span><var>|</var><em>파워볼사다리</em>
                        <select>
                            <option>전체보기</option>
                            <option>전체보기</option>
                            <option>전체보기</option>
                    ...
                    <div class="bl_live_betting_middle">
                        <table class="table-black minigame normal">
                            ...
                            <thead>
                                <tr> <!-- TITLE -->
                                    <th colspan="4">
                                        <!--<img src="/_go/renewal/img/icon_minigame.png" />--><span>파워사다리 게임</span><code><?php echo date("m-d",strtotime($rs['G_Datetime'])); ?><strong><?php echo date("H-i",strtotime($rs['G_Datetime'])); ?></strong></code>
                                    </th>
                                </tr>
                                <tr>
                                    <td>게임일시</td>
                            ...
                            <tbody>
                            <?php
                                $sql = "SELECT * FROM gamelist_other WHERE G_Num2 = '{$rs['G_Num2']}' AND DATE_FORMAT(G_Datetime,'%Y-%m-%d') = '{$startDate}' AND GI_Key = 2  ORDER BY GL_Key ASC LIMIT 5";
                                //echo $sql;
                                $list = getArr($sql);
